...
my $platform = "mysql";
my $port = "3306";
my $dbhost = "monitormysql1.no-ip.org";
my $database = "";
my $tablename = "";
my $query;
my $query_handle;
...
for($numSample=0;;$numSample++)
{
	...
	for(my $i=0;$i<=@host-1;$i++) 
	{
		...
		if (!defined $session) {
			...
			#my $Time = get_time();
			$query = "INSERT INTO monitorErrorLog VALUES (NULL, NULL, '$host[$i]', '$error')";
			$query_handle = $connect->prepare($query);
					...
					if($varresult < $tr1[1])
					{
						if($tr1[2] eq 'email')
						{
							...
							if($needmail eq 'N')
							{
								...
								$query_handle = $connect->prepare($query);
								$query_handle->execute();	
								$query = "INSERT INTO monitorErrorLog VALUES (NULL, NULL, '$iphost', '$tr1[3]')";
								$query_handle = $connect->prepare($query);
								$query_handle->execute();
								print "Email sent.\n";
							}
					...
					if($varresult < $tr2[1])
					{
						if($tr2[2] eq 'email')
						{
							...
							if($needmail eq 'N')
							{
								...
								$query_handle = $connect->prepare($query);
								$query_handle->execute();	
								$query = "INSERT INTO monitorErrorLog VALUES (NULL, NULL, '$iphost', '$tr2[3]')";
								$query_handle = $connect->prepare($query);
								$query_handle->execute();
								print "Email sent.";
							}
