...
while(@return_value = $query_handle->fetchrow_array())
{
	...
	#$sampleDayLimit = @return_value[11];
	#$sampleMonthLimit = @return_value[10];
}
...
#		}
	...
	$query = "UPDATE monitorConfig SET numberOfSample = '$numSample'";
	$query_handle = $connect->prepare($query);
	$query_handle->execute();
...
sub get_daydata
{
	...
	if($hour24 >= 3)
	{
		...
		#$query = "UPDATE monitorConfig SET numberOfHour = '$numSampleDay'";
		#$query_handle = $connect->prepare($query);
		#$query_handle->execute();
